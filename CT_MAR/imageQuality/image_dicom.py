...
class ImageAnalyzer(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle('DICOM 영상 질 비교(CNR, SNR, PSNR, SSIM)')

        # UI 세팅
        self.openButtonOriginal = QPushButton('원본 DICOM 선택', self)
        self.openButtonCompare = QPushButton('비교 DICOM 선택', self)
        self.openButtonProcessed = QPushButton('영상처리 DICOM 선택', self)
        ...
        # 연결
        self.openButtonOriginal.clicked.connect(self.openDICOMOriginal)
        self.openButtonCompare.clicked.connect(self.openDICOMCompare)
        self.openButtonProcessed.clicked.connect(self.openDICOMProcessed)
    ...
    #원본영상 열기
    def openDICOMOriginal(self):
        desktop_path = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
        fname, _ = QFileDialog.getOpenFileName(self, 'Open Original DICOM File', desktop_path, 'DICOM files (*.dcm)')
        if fname:
            dicom = pydicom.dcmread(fname)
            self.original_image = dicom.pixel_array
            self.displayImage(self.original_image, 'original')
            self.original_filename = fname  # 파일 이름을 저장하는 부분 추가

    #비교 노이즈 영상 열기
    def openDICOMCompare(self):
        desktop_path = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
        fname, _ = QFileDialog.getOpenFileName(self, 'Open Compare DICOM File', desktop_path, 'DICOM files (*.dcm)')
        if fname:
            dicom = pydicom.dcmread(fname)
            self.compare_image = dicom.pixel_array
            self.displayImage(self.compare_image, 'compare')
            
    #영상처리 된 영상 열기
    def openDICOMProcessed(self):
        desktop_path = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
        fname, _ = QFileDialog.getOpenFileName(self, 'Open Processed DICOM File', desktop_path, 'DICOM files (*.dcm)')
        if fname:
            dicom = pydicom.dcmread(fname)
            self.processed_image = dicom.pixel_array
