//====  Datasets setup  ===================================================================================
   std::string train_subdir = parser.get<cv::String>("train_dir"); //get Train sub-dir. like 'Train' or 'train'
   fs::path train_path = fs::path(dataset_dir) / fs::path(train_subdir); //full path to Train directory in the dataset
   std::string test_subdir = parser.get<cv::String>("test_dir"); //get Test sub-dir.
   fs::path test_path = fs::path(dataset_dir) / fs::path(test_subdir); //full path to Test directory in the dataset

   //  Train dataset
   Segmnt_Dataset train_dts(train_path.generic_string(), val_ratio, img_chnl, img_sz, /*shuffle*/ true); //Train dataset
   Segmnt_Dataset test_dts(test_path.generic_string(), 0.0, img_chnl, img_sz, /*shuffle*/ true); //Test dataset
   Segmnt_Dataset val_dts; 

   //set Validation and Test datasets
   if (val_ratio > 0.) {
      val_dts.copy_split(train_dts); //copy split data and all other params from Train dataset.
      if (test_subdir == "__Val__") {
         test_dts = val_dts;
      }
      else {
         test_dts = Segmnt_Dataset(test_path.generic_string(), 0.0, img_chnl, img_sz, /*shuffle*/ true); //Test dataset
      }
   }
   else if (test_subdir != "__Val__") {
      test_dts = Segmnt_Dataset(test_path.generic_string(), 0.0, img_chnl, img_sz, /*shuffle*/ true); //Test dataset
      //val_ratio == 0.0 
      val_dts = test_dts;
   }
   else {  // test_subdir == "__Val__" && val_ratio == 0.0
      val_dts = train_dts;
      test_dts = train_dts;
   }
   ...
   int n_classes = std::max(train_dts.n_classes(), test_dts.n_classes()); // for VOC2012 dataset = 22 classes

   int batch_size = parser.get<int>("batch_sz"); //if 0 => auto-fit from 1 to max. Batch_N, without torch error.
   int tr_wrk = parser.get<int>("tr_wrk");
   int trn_dts_sz = train_dts.size().value();
   int tst_dts_sz = test_dts.size().value();
   int val_dts_sz = val_dts.size().value();
   float val_share = round(1000. * val_dts_sz / (val_dts_sz + trn_dts_sz)) / 1000.; //leave 3 digits after comma.

   LOG(INFO) << "<<_Dir_Dataset_>> dir:'" << dataset_dir << "'"
      << "\n|img_chnl:" << img_chnl << " |img_sz: " << img_sz << " |n_classes:" << n_classes
      << " |batch_sz:" << batch_size
      << " |train_sz:" << train_dts.size().value() << " |val_sz:" << val_dts.size().value()
      << " |val_share: " << val_share
      ; 

   bool pretrained = parser.has("pretrained"); 


//

   class Segmnt_Dataset : public torch::data::datasets::Dataset<Segmnt_Dataset> {
prsize_ = 0; //output height and width for all image and mask tensors. If 0 => smallest size of 1st image.
   int n_channels_ =ivate: ///fields
   ...
   int out_tnz_ -1; //(auto) output N channels in photo images =3 for RGB or =1 for b/w images.
