        ...
        if len(Files) > 0:
            for File_Name in Files:
                if os.path.splitext(File_Name)[1] in Possible_Img_Extension:
                    ...
                    # alpha_list = np.arange(-5, -10, 0.1)
                    # for dhe_alpha in alpha_list:
                    #     dhe_Image = dhe(Input_dhe_Image, dhe_alpha)
                    #     dhe_Image_Path = Root + f'/{dhe_alpha}_dhe_' + File_Name
                    #     plt.imsave(dhe_Image_Path,dhe_Image)
                    #     print(f'$$ {dhe_Image_Path} is done')
                    
                    # A New Image Contrast Enhancement Algorithm Using Exposure Fusion Framework (ying)
                    ying_Folder_Path = Path + '\\ying'
                    CreateDirectory(ying_Folder_Path)
                    Input_ying_Image = imageio.imread(Img_Path)
                    # def Ying_2017_CAIP(img, mu=0.5, ying_a=-0.3293, ying_b=1.1258, lamda=0.5, sigma = 5):
                    mu_list = np.arange(0, 100, 1)
                    a_list = np.arange(-50, 50, 1)
                    b_list = np.arange(0, 100, 1)
                    lamda_list = np.arange(0, 10, 0.1)
                    sigma_list = np.arange(0, 100, 1)

                    # for ying_mu in mu_list:
                    #     ying_Image = Ying_2017_CAIP(Input_ying_Image, ying_b=ying_mu)
                    #     ying_Image_Path = ying_Folder_Path + f'/mu_{ying_mu}_ying_' + File_Name
                    #     plt.imsave(ying_Image_Path,ying_Image)
                    #     print(f'$$ {ying_Image_Path} is done')
                    
                    # for ying_alpha in a_list:
                    #     ying_Image = Ying_2017_CAIP(Input_ying_Image, ying_a=ying_alpha)
                    #     ying_Image_Path = ying_Folder_Path + f'/Alpha_{ying_alpha}_ying_' + File_Name
                    #     plt.imsave(ying_Image_Path,ying_Image)
                    #     print(f'$$ {ying_Image_Path} is done')
                        
                    # for ying_beta in b_list:
                    #     ying_Image = Ying_2017_CAIP(Input_ying_Image, ying_b=ying_beta)
                    #     ying_Image_Path = ying_Folder_Path + f'/Beta_{ying_beta}_ying_' + File_Name
                    #     plt.imsave(ying_Image_Path,ying_Image)
                    #     print(f'$$ {ying_Image_Path} is done')
                    for ying_lamda in lamda_list:
                        ying_Image = Ying_2017_CAIP(Input_ying_Image, lamda=ying_lamda)
                        ying_Image_Path = ying_Folder_Path + f'/lamda_{ying_lamda}_ying_' + File_Name
                        plt.imsave(ying_Image_Path,ying_Image)
                        print(f'$$ {ying_Image_Path} is done')
                    
                    for ying_sigma in sigma_list:
                        ying_Image = Ying_2017_CAIP(Input_ying_Image, sigma=ying_sigma)
                        ying_Image_Path = ying_Folder_Path + f'/sigma_{ying_sigma}_ying_' + File_Name
                        plt.imsave(ying_Image_Path,ying_Image)
                        print(f'$$ {ying_Image_Path} is done')
...
def entropy(X):
    ...
    return S
def maxEntropyEnhance(I, isBad, a, b):
    # Esatimate k
    ...
    isBad = isBad * 1
    isBad = np.array(Image.fromarray(isBad).resize((50, 50), resample=PIL.Image.BICUBIC))
    ...
    f = lambda k: -entropy(applyK(Y, k, a, b))
...
def Ying_2017_CAIP(img, mu=0.5, ying_a=-0.3293, ying_b=1.1258, lamda=0.5, sigma = 5):
    ...
    #new_scale = scipy.misc.imresize(t_b, 0.5, interp='bicubic', mode='F')
    t_our = cv2.resize(tsmooth(new_scale, lamda, sigma), (t_b.shape[1], t_b.shape[0]), interpolation=cv2.INTER_AREA)
    #t_our = t_b
    # Apply camera model with k(exposure ratio)
    isBad = t_our < 0.5
    J = maxEntropyEnhance(I, isBad, ying_a, ying_b)
